...
/////////////////////////////////////////////////////////////////////////////
/////////////////////////////////////////////////////////////////////////////////
//#define TRIGGER_PIN 32  // Pin Trigger
...
// Adicionalmente:
#define NUM_MEASUREMENTS 10 // Número de mediciones para promediar

float distance;             // Para almacenar la distancia calculada
float averageDistance=0.00;  
...
#define TXdbmW  20           // Transmit power in dBm
...
//extern uint8_t validMeasurements;

extern int validMeasurements; // Declaración de variab
// Archivo.cpp
...
void setup()
{
  ...
  pinMode(ECHO_PIN, INPUT);  // Configura el pin de Echo como entrada
  pinMode(TRIGGER_PIN, OUTPUT);  // Configura el pin de Trigger como salida
  ...
  #ifdef T_BEAM_V1_0
    writedisplaytext("LoRa-APRS","","Init:","ADC OK!","DIS: "+String(averageDistance,2),"",250);
    Serial.print("LoRa-APRS / Init / ADC OK! / Dis: ");
...
void loop() {
  ...
  /////////////////////////////////////////////////////////////////////////////
    ...
    float distanceSum = 0;  // Suma de distancias válidas
    int validMeasurements = 0;  // Contador de mediciones válidas
    ...
    for (int i = 0; i < NUM_MEASUREMENTS; i++) {
        ...
        // Lee la duración del pulso de Echo
        long duration = pulseIn(ECHO_PIN, HIGH, 50000); // Timeout de 30 ms
        if (duration == 0) {
            Serial.println("Pulso no detectado, descartando...");
        ...
        if (distance > 0 && distance <= MAX_DISTANCE) {
            ...
            Serial.println("Distancia fuera de rango, descartada.");
        }

        delay(200);  // Retraso entre mediciones
    }

    // Calcula el promedio solo si hay mediciones válidas
    if (validMeasurements > 0) {
        averageDistance = distanceSum / validMeasurements;
...
Serial.print("Frecuencia de transmisión (MHz): ");
Serial.println(TXFREQ);
Serial.print("Potencia de transmisión (dBm): ");

Serial.println("Configuración APRS:");
Serial.print("Indicativo principal (Tcall): ");
Serial.println(Tcall);
Serial.print("Indicativo meteorológico (wxTcall): ");
Serial.println(wxTcall);
Serial.print("Tabla de símbolos: ");
Serial.println(sTable);
Serial.print("Símbolo APRS: ");
Serial.println(wxSymbol);
Serial.println("Smart Beaconing:");
Serial.print("Tiempo mínimo entre transmisiones (ms): ");
Serial.println(min_time_to_nextTX);
Serial.print("Ángulo mínimo de cambio de curso para transmitir: ");
...
Serial.print("Ángulo mínimo de cambio de curso para transmitir: ");
Serial.println(ANGLE);
Serial.print("Modo actual del tracker: ");
Serial.println(tracker_mode);
  ...
  if ( (lastTX+nextTX) <= millis()  ) {
    if (tracker_mode != WX_FIXED) {
      ...
      if (gps.location.age() < 2000) {
        digitalWrite(TXLED, HIGH);
        if (hum_temp) {
          writedisplaytext(" ((TX))","","LAT: "+LatShown,"LON: "+LongShown,"SPD: "+String(gps.speed.kmph(),1)+"  CRS: "+String(gps.course.deg(),1),"DIS: "+String(averageDistance,2)+"  HUM: "+String(hum,1),0);
        } else {
        ...
        Serial.print(" / DIS: ");
        Serial.print(String(averageDistance,2));
        Serial.print(" / TEMP: ");
        ...
        if (hum_temp) {
          writedisplaytext(" "+Tcall,"(TX) at valid GPS","LAT: not valid","LON: not valid","SPD: ---  CRS: ---","DIS: "+String(averageDistance,2)+"  HUM: "+String(hum,1),0);
        } else {
      ...
      digitalWrite(TXLED, HIGH);
      if (hum_temp) {
        writedisplaytext(" ((TX))","","LAT: "+LatShown,"LON: "+LongShown,"No GPS used","DIS: "+String(averageDistance,2)+"  HUM: "+String(hum,1),0);
      } else {
      ...
      Serial.print(" / LON: ");
      Serial.print(LongShown);
      Serial.print(" / No GPS used / SAT: --- / DIS: ");
      Serial.print(String(averageDistance,2));
      Serial.print(" / TEMP: ");
      Serial.print(String(temp,1));
      Serial.print(" / HUM: ");
    ...
    if (tracker_mode != WX_FIXED) {
      if (gps.location.age() < 2000) {
        if (hum_temp) {
          writedisplaytext(" "+Tcall,"Time to TX: "+String(((lastTX+nextTX)-millis())/1000)+"sec","LAT: "+LatShown,"LON: "+LongShown,"SPD: "+String(gps.speed.kmph(),1)+"  CRS: "+String(gps.course.deg(),1),"DIS: "+String(averageDistance,2)+"  HUM: "+String(hum,1),0);
        } else {
        ...
        if (hum_temp) {
          writedisplaytext(" "+Tcall,"Time to TX: "+String(((lastTX+nextTX)-millis())/1000)+"sec","LAT: not valid","LON: not valid","SPD: ---  CRS: ---","DIS: "+String(averageDistance,2)+"  HUM: "+String(hum,1),0);
        } else {
      ...
      if (hum_temp) {
        writedisplaytext(" "+wxTcall,"Time to TX: "+String(((lastTX+nextTX)-millis())/1000)+"sec","LAT: "+LatShown,"LON: "+LongShown,"SPD: ---  CRS: ---","DIS: "+String(averageDistance,2)+"  HUM: "+String(hum,1),0);
      } else {
...
switch(tracker_mode) {
  case WX_FIXED:
    ...
    outString += MY_COMMENT;
      ...
      outString += (" --");
      outString += " Dis=";
      outString += String(averageDistance,2);
      outString += ("V");
    #endif
    break;
  case WX_TRACKER:
...
case WX_MOVE:
  ...
  default:
      ...
      outString += Speedx;
        ...
        outString += Altx;
        outString += " Dis=";
        outString += String(averageDistance,22);
        outString += ("V");
      #endif
      outString += MY_COMMENT;
      #ifdef DEBUG
...
switch(tracker_mode) {
  ...
  default:
    if ( gps.location.isValid()   || gps.location.isUpdated() ) {
      ...
      Outputstring += " Dis=";
      Outputstring += String(averageDistance,2);
      Outputstring += ("V ");
